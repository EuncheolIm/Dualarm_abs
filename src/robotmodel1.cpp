...
void CModel::Initialize()
{
	...
	_J_left_lowerArm.setZero(6,_k);
	_J_left_lowerArm_pos.setZero(3,_k);
	_J_left_lowerArm_ori.setZero(3,_k);

	_x_left_lowerArm.setZero(); // 3x1
	_xdot_left_lowerArm.setZero(); //3x1


	_J_left_lowerArm7.setZero(6,8);
	_J_left_lowerArm_pos7.setZero(3,8);
	_J_left_lowerArm_ori7.setZero(3,8);
	_J_tmp7.setZero(6,8);

	_q7.setZero(8);
	_qdot7.setZero(8);
	_x_leftLower.setZero();
	_xdot_leftLower.setZero();
...
{
	//read urdf model
	// RigidBodyDynamics::Addons::URDFReadFromFile("/home/kist/euncheol/Dualarm-MPC/model/dualarm_hands_mod_sim_fixed.urdf", &_model, false, true);	
	RigidBodyDynamics::Addons::URDFReadFromFile("/home/kist/euncheol/Dualarm-MPC/model/dualarm_hands_mod_sim.urdf", &_model, false, true);		

	//body id 1: body_link (trunk)
	//body id 8: LWrR_Link (left hand)
	//body id 15: RWrR_Link (right hand)

	cout << endl << endl << "Model Loaded for RBDL." << endl << "Total DoFs: " << _model_co.dof_count << endl << endl;
...
void CModel::calculate_EE_Jacobians()
{
	if (_bool_kinematics_update == true)
	{
		...
		_J_left_hand.block<3, 15>(0, 0) = _J_tmp.block<3, 15>(3, 0);
		_J_left_hand.block<3, 15>(3, 0) = _J_tmp.block<3, 15>(0, 0);
		_J_left_hand_pos =  _J_tmp.block<3, 15>(3, 0);
		_J_left_hand_ori =  _J_tmp.block<3, 15>(0, 0);
		...
		_J_right_hand.block<3, 15>(0, 0) =  _J_tmp.block<3, 15>(3, 0);
		_J_right_hand.block<3, 15>(3, 0) =  _J_tmp.block<3, 15>(0, 0);
		_J_right_hand_pos =  _J_tmp.block<3, 15>(3, 0);
		_J_right_hand_ori =  _J_tmp.block<3, 15>(0, 0);

		/// @brief //////////////////////////
		_J_left_lowerArm.setZero();
		_J_tmp.setZero();
		RigidBodyDynamics::CalcPointJacobian6D(_model, _q, _id_left_lowerarm, _position_local_task_left_hand_lower_arm, _J_tmp, false); //left_lowerArm
		_J_left_lowerArm.block<3, 15>(0, 0) = _J_tmp.block<3, 15>(3, 0);
		_J_left_lowerArm.block<3, 15>(3, 0) = _J_tmp.block<3, 15>(0, 0);
		_J_left_lowerArm_pos =  _J_tmp.block<3, 15>(3, 0);
		_J_left_lowerArm_ori =  _J_tmp.block<3, 15>(0, 0);

		_J_left_lowerArm7.setZero();
		_J_tmp7.setZero();
		RigidBodyDynamics::CalcPointJacobian6D(_model,_q7, _id_left_lowerarm, _position_local_task_left_hand_lower_arm, _J_tmp7, false); //left_lowerArm
		_J_left_lowerArm7.block<3, 8>(0, 0) = _J_tmp7.block<3, 8>(3, 0);
		_J_left_lowerArm7.block<3, 8>(3, 0) = _J_tmp7.block<3, 8>(0, 0);
		_J_left_lowerArm_pos7 =  _J_tmp7.block<3, 8>(3, 0);
		_J_left_lowerArm_ori7 =  _J_tmp7.block<3, 8>(0, 0);
...
void CModel::calculate_EE_positions_orientations()
{
	if (_bool_kinematics_update == true)
	{
		...
		_x_left_shoulder.setZero();
		_x_left_shoulder = RigidBodyDynamics::CalcBodyToBaseCoordinates(_model, _q, _id_left_shoulder, _position_local_zerovec, false);
		_x_right_shoulder.setZero();
		_x_right_shoulder = RigidBodyDynamics::CalcBodyToBaseCoordinates(_model, _q, _id_right_shoulder, _position_local_zerovec, false);

		///  /////////////////////////
		_x_left_lowerArm.setZero();
		_x_left_lowerArm = RigidBodyDynamics::CalcBodyToBaseCoordinates(_model, _q, _id_left_lowerarm, _position_local_task_left_hand_lower_arm, false);

		_x_leftLower.setZero();
		_x_leftLower = RigidBodyDynamics::CalcBodyToBaseCoordinates(_model, _q7, _id_left_lowerarm, _position_local_task_left_hand_lower_arm, false);
	}
	else
	{
		cout << "Robot kinematics is not ready. Please update kinematics first." << endl << endl;
...
void CModel::calculate_EE_velocity()
{
	if (_bool_Jacobian_update == true)
	{
		...
		_xdot_left_lowerArm = _J_left_lowerArm_pos * _qdot;
		_xdot_leftLower = _J_left_lowerArm_pos7 * _qdot7;
	}
	else
	{
...
void CModel::set_robot_config()
{
	...
	_position_local_task_left_hand_lower_arm.setZero();
	_position_local_task_left_hand_lower_arm(1) = -0.35;
